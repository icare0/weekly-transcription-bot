...
describe('splitAudioFile', () => {
  ...
  beforeAll(() => {
    if (!fs.existsSync(testFilePath))
      throw new Error(
        'test.mp3 file is missing. Please add it to the test directory.'
      );
  });
  ...
  it('should split a large MP3 file into exactly 3 parts and log file sizes', async () => {
    ...
    if (expectedParts < 3)
      throw new Error(
        `Test file is too small to be split into 3 parts. File size: ${fileSize_MB} MB, maxFileSize_MB: ${maxFileSize_MB}`
      );
  ...
  it('should throw an error for unsupported file formats', async () => {
    ...
    ).rejects.toThrow('Unsupported file format. Only MP3 files are supported.');
